...
{
	/// <summary>PascalsTriangle can be used for O(1) lookup of the nCr function.</summary>
	public sealed class PascalsTriangle
	{
		private PascalsTriangle()
		{
		}

		private static double[][] pt = new double[][] { new double[] { 1 } };

		/// <summary>The nCr function returns the number of ways r things can be chosen from a set of size n.</summary>
		/// <remarks>
		...
		public static double NCr(int n, int r)
		{
			lock (typeof(PascalsTriangle))
			{
				...
				if (n >= pt.Length)
				{
					...
					for (int i_1 = pt.Length; i_1 < pt2.Length; i_1++)
					{
						pt2[i_1] = new double[(i_1 / 2) + 1];
						pt2[i_1][0] = 1;
						for (int j = 1; j < pt2[i_1].Length; j++)
						{
							double x = pt2[i_1 - 1][j - 1];
							if (j < pt2[i_1 - 1].Length)
							{
								x = x + pt2[i_1 - 1][j];
							}
							else
		...
		/// <summary>Resets the internal array to the initial state to free up memory.</summary>
		public static void Reset()
		{
			lock (typeof(PascalsTriangle))
			{
				pt = new double[][] { new double[] { 1 } };
			}
		}
	}
}
