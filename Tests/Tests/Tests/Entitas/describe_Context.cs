    ...
    void when_created() {
        ...
        context["events"] = () => {
            ...
            it["dispatches OnGroupCreated when creating a new group"] = () => {
                ...
                var group = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(0));
                didDispatch.should_be(1);
                eventGroup.should_be_same(group);
            };

            it["doesn't dispatch OnGroupCreated when group alredy exists"] = () => {
                ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(0));
        ...
        context["entity pool"] = () => {
            ...
            it["sets up entity from pool"] = () => {
                ...
                e.Destroy();
                var g = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentA));
        ...
        context["groups"] = () => {
            ...
            context["when entities created"] = () => {
                ...
                IMatcher<TestEntity> matcherAB = Matcher<TestEntity>.CreateAllOf(new [] {
                    CID.ComponentA,
                    CID.ComponentB
                });
                ...
                it["gets group with matching entities"] = () => {
                    var g = ctx.AddGetGroup(matcherAB).GetEntities();
                ...
                it["gets cached group"] = () => {
                    ctx.AddGetGroup(matcherAB).should_be_same(ctx.AddGetGroup(matcherAB));
                };

                it["cached group contains newly created matching entity"] = () => {
                    var g = ctx.AddGetGroup(matcherAB);
                    eA.AddComponentB();
                ...
                it["group dispatches OnEntityUpdated with previous and current component when replacing a component"] = () => {
                    ...
                    var g = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentA));
                    g.OnEntityUpdatedWithCmp += (group, entity, index, previousComponent, newComponent) => {
                        updated += 1;
                        group.should_be_same(g);
                ...
                it["group with matcher NoneOf doesn't dispatch OnEntityAdded when destroying entity"] = () => {
                    var e = ctx.CreateEntity()
                                .AddComponentA()
                                .AddComponentB();
                    var matcher = Matcher<TestEntity>.CreateAllOf(CID.ComponentB).NoneOf(CID.ComponentA);
                    var g = ctx.AddGetGroup(matcher);
                ...
                context["event timing"] = () => {
                    ...
                    it["dispatches group.OnEntityAdded events after all groups are updated"] = () => {
                        var groupA = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentA, CID.ComponentB));
                        var groupB = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentB));
                    ...
                    it["dispatches group.OnEntityRemoved events after all groups are updated"] = () => {
                        ctx = new MyTestContext();
                        var groupB = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentB));
                        var groupAB = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentA, CID.ComponentB));
        ...
        context["EntityIndex"] = () => {
            ...
            it["adds and EntityIndex"] = () => {
                ...
                var entityIndex = new PrimaryEntityIndexer<TestEntity, string>("TestIndex", ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(componentIndex)), (arg1, arg2) => string.Empty);
        ...
        context["EntitasCache"] = () => {
            ...
            it["pops new list from list pool"] = () => {
                var groupA = ctx.AddGetGroup(Matcher<TestEntity>.CreateAllOf(CID.ComponentA));
                var groupAB = ctx.AddGetGroup(Matcher<TestEntity>.CreateAnyOf(CID.ComponentA, CID.ComponentB));
                var groupABC = ctx.AddGetGroup(Matcher<TestEntity>.CreateAnyOf(CID.ComponentA, CID.ComponentB, CID.ComponentC));
